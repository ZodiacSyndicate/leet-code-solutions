...
var findOrder = function(numCourses, prerequisites) {
  const clen = prerequisites.length
  if (clen === 0) return Array.from({ length: numCourses }, (_, i) => i)
  ...
  while (queue.length) {
    ...
    for (const succesor of adj[top].values()) {
      degrees[succesor]--
      if (degrees[succesor] === 0) {
        queue.push(succesor)
      }
    }
  }
  return counter === numCourses ? res : []
}
